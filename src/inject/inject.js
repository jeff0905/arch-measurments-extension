...
/**
 * 根据dom,css selector 来定位dom对象, 显示该对象的辅助线.
 * 
 * @param {string} selector 选择符,
 * 
 */
oPageLiner.addDom = function(selector) {
    var self = this;
    var domlist = $(selector);
    console.log('selector', selector, domlist);

    domlist.each(function(index, item) {
        
            var dimensions = self.getDomDimensions(item);
        console.log(dimensions);
        var posX = dimensions.offset.left;
        var posY = dimensions.offset.top;
        var sColor = '#1890ff';
        
        // vertical
        var oHelpLine = self.addRule(dimensions, sColor, self.addHelpLineToLocalStorage(posX, posY, sColor));
        var oHelpLine2 = self.addRule(dimensions, sColor, self.addHelpLineToLocalStorage(posX, posY, sColor));
        
        var oHelpLine3 = self.addRule(dimensions, sColor, self.addHelpLineToLocalStorage(posX, posY, sColor));
        var oHelpLine4 = self.addRule(dimensions, sColor, self.addHelpLineToLocalStorage(posX, posY, sColor));
        
        self.updatePopUp();
    
        return [oHelpLine, oHelpLine2, oHelpLine3, oHelpLine4];
        
    })
    return selector;
}

oPageLiner.getDomDimensions = function(dom) {
    
    var contentWidth = $(dom).width();
    var contentHeight = $(dom).height();
    var padding = {
        top: $(dom).css('padding-top'),
        right: $(dom).css('padding-right'),
        bottom: $(dom).css('padding-bottom'),
        left: $(dom).css('padding-left'),
    }
    var margin = {
        top: $(dom).css('margin-top'),
        right: $(dom).css('margin-right'),
        bottom: $(dom).css('margin-bottom'),
        left: $(dom).css('margin-left'),
    }
    var wh = {
        width: $(dom).width(),
        height: $(dom).height(),
 ...
 * @param {*} scolor 
 */
oPageLiner.addRule = function(dimensions,scolor,iHelplineIndex) {
    var posX = dimensions.offset.left;
    var posY = dimensions.offset.top;
    var width = dimensions.wh.width;
    var height = dimensions.wh.height;
    var padding = dimensions.padding;
    var margin = dimensions.margin;
    var realWidth = parseFloat(padding.left) + width + parseFloat(padding.right);
    var realHeight = parseFloat(padding.top) + height + parseFloat(padding.bottom);
    console.log(padding.left, width, padding.right, realWidth);
    var $window = $(window),
    oHelpLine =
        {
            posX: typeof posY !== 'undefined' && typeof posY !== 'undefined' ? posX : 0,
    ...
    oHelpLineElem.className = 'ruler-line';
    ...
    // 宽高.
    var style="top:" + posY + "px;left:"+posX + "px;width="+ realWidth + "px;height="+realHeight+"px";
    oHelpLineElem.style.width = realWidth + 'px';
    oHelpLineElem.style.height = 1 + 'px';
    oHelpLineElem.style.top = posY + 'px';
    oHelpLineElem.style.left = posX + 'px';
    
    // 左边 posX, posY, w:1, h: realheight.
    var rleft = document.createElement('div');
    rleft.className = 'ruler-line';
    rleft.style.backgroundColor = oHelpLine.sColor;
    rleft.setAttribute('data-pglnr-ext-helpline-index', iHelplineIndex + 1);
    
    rleft.style.width = 1 + 'px';
    rleft.style.height = realHeight + 'px';
    rleft.style.top = posY + 'px';
    rleft.style.left = posX + 'px';

    // 右边, 
    var rright = document.createElement('div');
    rright.className = 'ruler-line';
    rright.style.backgroundColor = oHelpLine.sColor;
    rright.setAttribute('data-pglnr-ext-helpline-index', iHelplineIndex + 1);
    
    rright.style.width = 1 + 'px';
    rright.style.height = realHeight + 'px';
    rright.style.top = posY + 'px';
    rright.style.left = posX + realWidth + 'px';

    // bottom
    
    var rbottom = document.createElement('div');
    rbottom.className = 'ruler-line';
    rbottom.style.backgroundColor = oHelpLine.sColor;
    rbottom.setAttribute('data-pglnr-ext-helpline-index', iHelplineIndex + 1);
    
    rbottom.style.width = realWidth + 'px';
    rbottom.style.height = 1 + 'px';
    rbottom.style.top = posY + realHeight + 'px';
    rbottom.style.left = posX + 'px';


    //标注, 宽高.
    var ruler = document.createElement('div');
    ruler.className="ruler-line";
    var rulerFirst = document.createElement('div');
    rulerFirst.className="ruler-first";
    var rulerLast = document.createElement('div');
    rulerLast.className="ruler-last";
    rulerLast.style.top = realHeight + 'px';
    var rulerText = document.createElement('div');
    rulerText.className="ruler-text";
    rulerText.innerText = realWidth + " X " + realHeight;

    ruler.style.left = posX + realWidth / 2 - 20 + 'px';
    ruler.style.top = posY + 'px';
    ruler.style.height = realHeight +'px';
    ruler.style.width = '1px';
    ruler.style.background = 'red';
    ruler.style.borderLeft="1px dotted red";
    ruler.appendChild(rulerFirst);
    ruler.appendChild(rulerText);
    ruler.appendChild(rulerLast);
    // 2. tooltip
